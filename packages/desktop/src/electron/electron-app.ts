...
const addScopedAppListener = (
  ...
  Effect.acquireRelease(
    Effect.sync(() => {
      type AppListener = Parameters<Electron.App["on"]>[1];
      Electron.app.on(eventName as Parameters<Electron.App["on"]>[0], listener as AppListener);
    }),
    () =>
      Effect.sync(() => {
        type AppListener = Parameters<Electron.App["on"]>[1];
        Electron.app.removeListener(
          eventName as Parameters<Electron.App["removeListener"]>[0],
          listener as AppListener,
        );
      }),
  ).pipe(Effect.asVoid);
